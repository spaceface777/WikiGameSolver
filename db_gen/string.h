...
public:
	...
	inline string(const string& a) : ptr(a.ptr), len(a.len) {
		LOG;
	};
	...
	inline string(const char* ptr, uint16_t len) : ptr((uintptr_t)ptr), len(len) {
		LOG;
	};

	inline string(const char* ptr) : ptr((uintptr_t)ptr), len(ptr == 0 ? 0 : (uint16_t)strlen(ptr)) {
		LOG;
	};
	...
	inline bool operator==(const string& other) const {
		return len == other.len && memcmp((const char*)(uintptr_t)ptr, (const char*)other.ptr, len) == 0;
	}
	...
	inline bool operator<(const string& other) const {
		int cmp = memcmp((const char*)(uintptr_t)ptr, (const char*)other.ptr, len < other.len ? len : other.len);
	...
	inline char operator[](int i) const {
		return ((const char*)(uintptr_t)ptr)[i];
	}
	...
	int find(char c, int start = 0) const {
		...
		const char* p = (const char*)memchr((char*)(uintptr_t)ptr + start, c, len - start);
		if (p == 0) return -1;
